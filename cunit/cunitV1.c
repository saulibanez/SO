...
comparefiles(char *file1, char *file2)
{
	...
	do{
		...
		if(strncmp(buffer1, buffer2, 1024)!=0){
			return -1;
		}
...
static void
executeoutpipes(int ncmd, char *name)
{
	int i;
	int pid;
	int status;
	...
	for (i = 0; i < ncmd; i++){
		...
		case 0:
			...
			tokenize(name, aux, ".");		//hago esto para quedarme en aux con el nombre del fichero sin extension

			sprintf(fout, "%s/%s.out", route, aux[0]);	//obtengo el nombre del fichero con .out
			fd = creat(fout, 0660);								//Creo el fichero .out y luego redirjo las salidas
			if(fd<0){
				err(1, "creat failed");
			}
			dup2(fd, 1);
			...
			// La entrada del test tiene que estar redirigida a /dev/null.
			if (i==0){
...
executecmd(int ncmd, char *name)
{
	...
	for (i = 0; i < ncmd; i++){
		...
		case 0:
			...
			// El fichero .out debe contener la salida estándar y la salida de errores del test
			if(i==ncmd-1){
				sprintf(fout, "%s/%s.out", route, aux[0]);	//obtengo el nombre del fichero con .out
				fd = creat(fout, 0660);								//Creo el fichero .out y luego redirjo las salidas
				if(fd<0){
...
savecmdtst(char *name)
{
	...
	for(;;){
		...
		if(strlen(buffer)==1){		//si en el fichero hay intros
			continue;
		}
		...
		if(strcmp(aux[0], "cd")==0){
			if(ninstructions==1){
				home = getenv("HOME");
				chdir(home);	//me cambia el directorio de trabajo
			}else{
				if(chdir(aux[1])<0){
					exit(EXIT_FAILURE);
				}
